...
func TestSeratService_DeleteSerat(t *testing.T) {
	...
	tests := []scenario{
		...
		},
		{
			...
			out: &output{
				res: nil,
				err: fmt.Errorf("DeleteSeratUseCase.Exec: failed to run usecase: bababoey"),
			},
			on: func(m *MockSeratService, in *input, out *output) {
				m.DeleteSeratUseCase.On("Exec", in.ctx, mock.AnythingOfType("uuid.UUID")).Return(out.err)
			},
		},
		{
			name: "OK",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &MockSeratService{
				DeleteSeratUseCase: new(mockUseCase.DeleteSeratUseCaser),
			}
